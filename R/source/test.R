...
# Wybierziris# Wybierz drugą kolumnę i wszystkie wiersze
kwiatki[,2]
...
kwiatki[c(4,8,22), c("Petal.Length","Sepal.Width")]
...
kwiatki$Petal.Iloraz<- kwiatki$Petal.Length / kwiatki$Petal.Width
...
data_2<- read_csv("R/data/zab_limnology.csv")

# Wczytaj dane z pliku excel na dysku, wyraźnie wskaż arkusz
data_3<-read_excel("R/data/Zmienne.xlsx", sheet = "Kroki")
# {dplyr} i {tidyr}
...
rename_with(pingwinki, .cols = everything(), \(x) toupper(x))
...
filter(pingwinki, gatunek %in% "Adelie" "Gentoo")
...
mutate(pingwinki, bill_ratio = bill_length_mm / bill_depth_mm)
  # Utwórz kolumnę id z numerem obserwacji (tip: 1:n())
mutate(pingwinki, id = 1:n())

# Zlogarytmuj w miejscu kolumnę body_mass_g
mutate(pingwinki, body_mass_g = log(body_mass_g))
# Oblicz w miejscu pierwiastek ze wszystkich kolumn numerycznych
mutate(pingwinki, across(where (\(y) is.numeric(y)), \(y) sqrt(y)))
...
summarise(pingwinki, across(c("bill_length_mm", "body_mass_g"), \(m) median(m, na.rm=TRUE)), .by = gatunek)

  # Zmień format ramki na dłuższy, wykorzystaj kolumny bill_length_mm i bill_depth_mm
dlugie<-pivot_longer(pingwinki, c("bill_length_mm", "body_mass_g"), names_to = "Nazwa", values_to = "Wartosci")

  # Zmień format ramki na szerszy, tak aby każda zmienna była wyrażona dla każdej wyspy
szerokie<-pivot_wider(pingwinki, names_from = island, values_from = "Wartosci")
...
left_join(data_5, data_7, by = join_by(sample.id))
...
right_join(data_6, data_7, by = join_by(sample.id))


# Korzystając z danych geochemicznych połącz ze sobą wszystkie trzy ramki w sposób całkowity
full_join(data_5, data_6, by = join_by(sample.id))|>
  full_join(data_7, by= join_by(sample.id))

# Oblicz w miejscu pierwiastek ze wszystkich kolumn numerycznych
mutate(pingwinki, across(where (\(k) is.numeric(k)), \(k) sqrt(k)))

# Zgrupuj ramkę ranych na podstawie zmiennej gatunek i wykonaj operację summarise (średnia) dla kolumn bill_length_mm i body_mass_g
group_by(pingwinki, gatunek)|>
  summarise(across(c("bill_length_mm", "body_mass_g"), \(m) mean(m, na.rm=TRUE))


#by

summarise(pingwinki, across(c("bill_length_mm", "body_mass_g"), \(e) median(e, na.rm = TRUE)), .by = island)

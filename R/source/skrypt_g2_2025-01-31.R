...
data[c(4,8,22), c("Sepal.Width", "Petal.Length")]
...
rename_with(.data = pingwinki,.cols = everything(), \(a) toupper (a))
...
select(pingwinki, dplyr::ends_with("mm"))
...
# Wybierz wiersze zawierające gatunek Adeline
filter(pingwinki, gatunek == "Adelie")

# Wybierz wiersze zawierające gatunek Adeline i Gentoo
filter(pingwinki, gatunek %in% c("Adelie", "Gentioo"))
...
mutate(pingwinki, across(where(\(kolumna) is.numeric(kolumna)), \(kolumna) sqrt(kolumna)))
...
test<-pivot_longer(pingwinki, c(bill_length_mm, bill_depth_mm), names_to = "nazwa", values_to = "wartosci")
...
#wczytaj arkusze od zarczynskiego
